...
# Add custom commands
echo "Would you like to add custom claude-code commands? (y/n)"
read -r add_commands
...
EOMD
    ...
    echo "Skipping custom command creation."
fi
...
if [ $? -eq 0 ]; then
    ...
    echo "Would you like to run the configuration script now? (y/n)"
    read -r configure
    if [[ $configure =~ ^[Yy]$ ]]; then
        ./configure.sh
    else
        echo "You can run the configuration script later with:"
        echo "  ~/projects/claude/claude-code/configure.sh"
    fi
else
    echo -e "${RED}Claude Code installation failed.${NC}"
    echo "Please check the installation logs and try again."
...
# Create additional files for Claude Code projects
echo "Creating additional files for Claude Code projects..."

# Create a template .claud.yml file
